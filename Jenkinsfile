...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="aoshi2/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/aoshi2/guestbook.git'
            }
        }
        stage('Build') {
            steps {
            ...
            steps{
                ...
                withSonarQubeEnv('SonarQube-Server'){
                    sh '''
                        ...
                        -Dsonar.host.url=http://192.168.56.140:9000 \
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_aoshi2') {
                        oDockImage.push()
                    }
                }
            }
